...
def get():
	...
	if dev.bcdDevice != REV_NUMBER:
		raise ValueError(f"device is not correct revision ({dev.bcdDevice >> 8 :X}.{dev.bcdDevice & 0xFF :02X} instead of {REV_NUMBER >> 8 :X}.{REV_NUMBER & 0xFF :02X})")
